	...
	public sealed class StoryGameEngine
	{
		...
		public StoryTurnResult HandleInput(string playerInput)
		{
			...
			var candidates = CurrentScene.Rules
				...
				.Where(r => IsMatch(normalized, r.TriggerVerbs))
				.OrderByDescending(r => r.TriggerVerbs.Max(v => v.Length))
				.ToList();
			...
			if (candidates.Count == 0)
			{
				...
				{
					candidates.Add(semanticMatch.Rule);
				}
			}

			if (candidates.Count == 0)
			{
				return BuildResult("Questa azione non sembra utile in questo punto della storia.", isBlocked: true, ruleId: null);
			}

			foreach (var rule in candidates)
			{
				if (!HasRequirements(rule))
				{
					string blockedText = string.IsNullOrWhiteSpace(rule.BlockedResponse)
						? "Ci provi, ma ti manca qualcosa per riuscirci."
						: rule.BlockedResponse;
					return BuildResult(blockedText, isBlocked: true, ruleId: rule.Id);
				}

				ApplyRule(rule);
				string response = BuildNarrativeResponse(rule.SuccessResponse, sceneChanged: rule.NextSceneId is not null);
				return BuildResult(response, isBlocked: false, ruleId: rule.Id);
			}

			return BuildResult("L'azione non produce effetti narrativi rilevanti.", isBlocked: true, ruleId: null);
		}

		private bool IsMatch(string normalizedInput, IReadOnlyList<string> verbs)
		{
		...
		private string BuildStartMessage()
		{
			...
			sb.AppendLine();
			sb.Append("Digita 'aiuto' per i comandi di supporto.");
			return sb.ToString();
		}
		...
		private string BuildFlagsMessage()
		{
			...
			{
				return "Nessun avanzamento narrativo registrato.";
			}
